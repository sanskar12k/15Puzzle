...
function shuffle() { 
    ...
    for( row = 1;row<=4;row++){
        for( column = 1;column<=4;column++){
            let row2 = Math.round(Math.random()*4 + 1);
            let column2 = Math.round(Math.random()*4 + 1);
            while(row2 == row && column == column2){
                row2 = Math.round(Math.random()*4 + 1);
                column2 = Math.round(Math.random()*4 + 1);
            }
...
var columnss;
function upKey(){
    for(rowss=2;rowss < 5;rowss++){
        for(columnss=1;columnss<5;columnss++){
            if(document.getElementById("cell"+rowss+columnss).className == "tile16"){
 ...
 function leftKey(){
      for(rowss=1;rowss < 5;rowss++){
          for(columnss=2;columnss<5;columnss++){
             if(document.getElementById("cell"+rowss+columnss).className == "tile16"){
...
function checkWin(){
     win = 0;
     if(winchance > 0 ){
        for(rowsss=1;rowsss<5;rowsss++){
            for(columnsss=1;columnsss<5;columnsss++){
              let  tilepos = (4*(rowsss-1))+columnsss;
      ...
      if(win == 16 ){
         ...
         if(timeshown == 1){
             ...
             if(typeof(Storage) !== 'undefined'){
               ...
               para.innerHTML = localStorage.getItem('hS[j]')+"s"; 
